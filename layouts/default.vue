...
export default {
  ...
  head() {
    return {
      meta: [
        ...
        { property: "og:title", content: "ECommerce" },
        {
          property: "og:description",
          content:
            "Electronic devices, gadgets and accessories shop. | Buy Acer, Dell, Asus, Razer, Apple laptops and ultrabooks at affordable prices"
        },
        { property: "twitter:title", content: "ECommerce" },
        {
  ...
  mounted() {
    ...
    if (process.client) {
      const savedTheme =
        window && window.localStorage.getItem("e-commerce-theme");
    ...
    if (process.client) {
      if (window.localStorage.getItem("e-commerce-loggedin")) {
  ...
  methods: {
    async fetchLoggedInUser() {
      if (this.attemptedLogin && !this.user) {
        window.localStorage.setItem("e-commerce-loggedin", null);
        return;
      }
      try {
